...
/**
 * Generate a folder name from contact ID (just the number)
 * This makes it easy to identify users during development
 */
export function getUserFolderNameFromContactId(contactId) {
  return String(contactId); // Just the number, e.g., "35"
}

/**
 * Generate a unique, privacy-safe folder name from email (legacy - kept for backward compatibility)
 */
...
/**
 * Get or create a user-specific folder using contact ID
 * Structure: TurnkiiClientUploads/{contactId}/
 */
export async function getUserFolderById(accessToken, contactId) {
  const folderName = getUserFolderNameFromContactId(contactId);
...
/**
 * Get or create a user-specific folder using email (legacy - kept for backward compatibility)
...
/**
 * Upload a file to OneDrive using contact ID for folder structure (RECOMMENDED)
 * Folder structure: TurnkiiClientUploads/{contactId}/
 */
export async function uploadToOneDriveById(accessToken, contactId, filename, fileBuffer) {
  console.log('🔍 uploadToOneDriveById: Starting upload...');
  console.log('  Contact ID:', contactId);
  console.log('  Filename:', filename);
  console.log('  File size:', fileBuffer.length, 'bytes');

  try {
    // Get user-specific folder using contact ID
    const userFolder = await getUserFolderById(accessToken, contactId);
    ...
    console.error('❌ uploadToOneDriveById error:', error);
    throw new Error(`Failed to upload file to OneDrive: ${error.message}`);
  }
}

/**
 * Upload a file to OneDrive (legacy - uses email, kept for backward compatibility)
 */
export async function uploadToOneDrive(accessToken, email, filename, fileBuffer) {
  ...
  try {
    // Get user-specific folder using email (legacy)
...
/**
 * List files for a specific user using contact ID
 */
export async function listUserFilesById(accessToken, contactId) {
  console.log(`🔍 listUserFilesById: Getting files for user: ${contactId}`);
...
/**
 * List files for a specific user (legacy - uses email)
 */
...
/**
 * Delete a file from OneDrive by file ID
 */
...
export default {
  ...
  getUserFolderById,
  getUserFolderName,
  getUserFolderNameFromContactId,
  uploadToOneDrive,
  uploadToOneDriveById,
  listFilesInFolder,
  listUserFiles,
  listUserFilesById,
  deleteFile,
